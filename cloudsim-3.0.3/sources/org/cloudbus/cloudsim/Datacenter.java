...
public class Datacenter extends SimEntity {
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			if((helperClass.getAlgorithmType() == helperClass.HONEYBEEWITHLOTTERY)||
					(helperClass.getAlgorithmType() == helperClass.LOTTERY))
			{
				int vmfromlottery = CloudletLoadBalancer.LotteryBasedBalancer(host.getVmList(),cl);
				int movePropertieslottery[] = {cl.getCloudletId(), userId, vmId, vmfromlottery, getId()};
				if((vmfromlottery!=vmId)&&(vmfromlottery!=-1))
				{
					sendNow(getId(), CloudSimTags.CLOUDLET_MOVE, movePropertieslottery);
					assignedVM.setVM(vmfromlottery);
				}
				else
				 assignedVM.setVM(vmId);
			}
			if((helperClass.getAlgorithmType() == helperClass.HONEYBEEWITHLOTTERY)||
					(helperClass.getAlgorithmType() == helperClass.HONEYBEEALONEWITHFCFSTYPE) || 
					(helperClass.getAlgorithmType() == helperClass.HONEYBEEALONEWITHUNIDIRECTION))
			{
				if(helperClass.getAlgorithmType() == helperClass.HONEYBEEWITHLOTTERY)
				{
				...
				{
					int moveProperties[] = {cl.getCloudletId(), userId, vmId, assignedVM.getVM(), getId()};
					if(assignedVM.getVM()!=vmId)
						sendNow(getId(), CloudSimTags.CLOUDLET_MOVE, moveProperties); 
				}
				int vmDestId = CloudletLoadBalancer.HBLoadBalancer(host.getVmList(),cl);
				if(vmDestId!=-1)
				{
					
					int moveProperties1[] = {cl.getCloudletId(), vm.getUserId(), vm.getId(), assignedVM.getVM(), getId()};
					if(vm.getId()!=assignedVM.getVM())
						sendNow(getId(), CloudSimTags.CLOUDLET_MOVE, moveProperties1); 
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for (Vm vm : host.getVmList()) {
				while (vm.getCloudletScheduler().isFinishedCloudlets()) {
					...
					if (cl != null) {
						//Log.printLine( " : host baba " + CloudSim.clock() + ": " + " cost is computed HB Here");
						//CloudletLoadBalancer.setStartTimerForCost(host.getVmList());
						sendNow(cl.getUserId(), CloudSimTags.CLOUDLET_RETURN, cl);
					}
				}
			}
